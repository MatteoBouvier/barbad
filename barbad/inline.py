    ...
    def visit_FunctionDef(self, node: ast.FunctionDef) -> None:
        match node:
            case ast.FunctionDef(
                name, _, _, _, ast.Subscript(ast.Name("Inline__", _), _, _), _
            ):
    ...
    def visit_Call(self, node: ast.Call):
        match node:
            case ast.Call(ast.Name(name, _), _, _) if name in self.inline_functions:
                ...
                args_translation = {
                    k.arg: v.id for k, v in zip(if_node.args.args, node.args)
                }
                ...
                return type(return_node)(
                    *(
                        ast.Name(
                            args_translation[field.id],
                            field.ctx,
                            lineno=field.lineno,
                            col_offset=field.col_offset,
                        )
                        if isinstance(field, ast.Name)
                        else field
                        for field in map(
                            lambda f: getattr(return_node, f), return_node._fields
                        )
                    ),
